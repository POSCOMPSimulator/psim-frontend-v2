...
const Errada = styled(Alternativa)`
		...
		return (
			<List selection size='large'>
				{questao.alternativas.map(
					(resp, ind) => {
						return ind === questao.resposta ?
							<Resposta key={ind}>
								<List.Content>
									<CustomRadio value={ind} checked={questao.alternativa_marcada === ind} />
									<InlineTex texContent={resp} />
								</List.Content>
							</Resposta> :
							(
								ind === questao.alternativa_marcada ?
								<Errada key={ind}>
									<List.Content>
										<CustomRadio value={ind} checked={questao.alternativa_marcada === ind} />
										<InlineTex texContent={resp} />
